...
#pragma region TextLine
	...
	void D2DText::DrawString(const std::string str, D2D1_POINT_2F pos, D2D1_DRAW_TEXT_OPTIONS options)
	{
		...
		switch (Origin) {
		case D2DText::ORIGIN::LEFTTOP:
			points.x = srcpos.x;
			points.y = srcpos.y;
			break;
		case D2DText::ORIGIN::RIGHTTOP:
			points.x = TargetSize.width + srcpos.x;
			points.y = srcpos.y;
			break;
		case D2DText::ORIGIN::LEFTBOTTOM:
			points.x = srcpos.x;
			points.y = TargetSize.height + srcpos.y;
			break;
		case D2DText::ORIGIN::RIGHTBOTTOM:
			points.x = TargetSize.width + srcpos.x;
			points.y = TargetSize.height + srcpos.y;
			break;
		case D2DText::ORIGIN::CENTER:
...
#pragma region TextParam
	...
	D2DTextParams::~D2DTextParams()
	{
		Manager->Remove(this);
		Manager = nullptr;
		SAFE_DELETE(TextSetting->Data);	//下の命令で消える？よくわからん
		SAFE_DELETE(TextSetting);
